...
char* const revision = "Firmware Revision 07_27_2011\r\n\0";
	...
	//		WriteSingle(temp, 2);
		...
		case 0xA1:									// 0xA1 - WUPA
			...
			*pbuf ^= 0x01;
			...
			Iso14443aAnticollision(*pbuf);  //JDW added this extra call because  
										    //basically the Anti-collision loop logic needs reworked
			break;
...
HostRequestCommand(u08_t *pbuf, u08_t length, u08_t broken_bits, u08_t no_crc)
{
	...
	*(pbuf + 4) = (rxtx_state << 4) | broken_bits;
	...
	{
		COUNT_VALUE = COUNT_1ms * 14;		// for TIMEOUT
		START_COUNTER;						// start timer up mode
		irq_flag = 0x00;
		while(irq_flag == 0x00)
		{
